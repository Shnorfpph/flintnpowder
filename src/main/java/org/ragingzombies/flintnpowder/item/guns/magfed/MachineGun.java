...
public class MachineGun extends MagfedBaseEnchantable {
    public MachineGun(Properties pProperties) {
        ...
        this.addCompatibleAttachmentTag("bipodable");
    }

    @Override
    public float accuracyModifier(LivingEntity ply, ItemStack gun) {
        return 4F * super.accuracyModifier(ply, gun);
    }
    ...
    public void onShoot(float rotationX, float rotationY, Level pLevel, LivingEntity shooter, ItemStack gunStack) {
        ...
        pLevel.playSeededSound(null, shooter.getBlockX(), shooter.getBlockY(), shooter.getBlockZ(),
                ModSounds.MACHINEGUN.get(), SoundSource.NEUTRAL, 3.0F, 1.0F, 0);
        pLevel.playSeededSound(null, shooter.getBlockX(), shooter.getBlockY(), shooter.getBlockZ(),
                ModSounds.GUNSHOTDISTANTHEAVY.get(), SoundSource.NEUTRAL, 9.0F, 1.0F, 0);

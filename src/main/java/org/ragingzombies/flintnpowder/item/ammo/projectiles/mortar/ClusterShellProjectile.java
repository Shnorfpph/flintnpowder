...
public class ClusterShellProjectile extends AbstractArrow implements ItemSupplier {

    MortarFallingSoundInstance snd = null;
    public ClusterShellProjectile(EntityType<? extends AbstractArrow> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }
    public ClusterShellProjectile(Level pLevel) {
        super(ModProjectiles.HESHELL.get(), pLevel);
    }
    public ClusterShellProjectile(Level pLevel, LivingEntity livingEntity) {
    ...
    void impact(BlockPos pos) {
        ...
        this.level().explode(this, null, null, getX(), getY(), getZ(), 3f, false, Level.ExplosionInteraction.NONE);

        int clusterSize = 15;

        for (int i = 0; i < clusterSize; i++) {
            double vx = (this.random.nextFloat() - 0.5F) * 1.15F;
            double vy = this.random.nextFloat() * 0.5F + 0.35F;
            double vz = (this.random.nextFloat() - 0.5F) * 1.15F;

            ClusterPellet pellet = new ClusterPellet(this.level(), (LivingEntity) this.getOwner());
            pellet.setPos(this.position());
            pellet.addDeltaMovement(new Vec3(vx, vy, vz));

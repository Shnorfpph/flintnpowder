...
import org.ragingzombies.flintnpowder.item.ammo.projectiles.mortar.HealingGasProjectile;
import org.ragingzombies.flintnpowder.item.ammo.projectiles.mortar.PoisonGasProjectile;

import javax.annotation.Nullable;
import java.util.List;

public class MortarGasHealingAmmo extends BaseAmmo {
    public MortarGasHealingAmmo(Properties pProperties) {
        super(pProperties);

        addRequiredTag("mortarold");
    }

    @Override
    public void onAmmoShot(float xRotation, float yRotation, LivingEntity shooter, ItemStack gun, Level level) {
        HealingGasProjectile proj = new HealingGasProjectile(level, shooter);

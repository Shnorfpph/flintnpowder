...
public class HealingGasProjectile extends AbstractArrow implements ItemSupplier {
    MortarFallingSoundInstance snd = null;
    public HealingGasProjectile(EntityType<? extends AbstractArrow> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }
    public HealingGasProjectile(Level pLevel) {
        super(ModProjectiles.HESHELL.get(), pLevel);
    }
    public HealingGasProjectile(Level pLevel, LivingEntity livingEntity) {
    ...
    void impact(BlockPos pos) {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 5; x++) {
                ...
                smoke.setColor(110, 0, 0);
                smoke.effect = MobEffects.REGENERATION;
